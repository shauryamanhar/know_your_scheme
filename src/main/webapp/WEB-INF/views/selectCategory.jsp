...
if(request.getAttribute("verifiedUser") != null &&
   !request.getAttribute("verifiedUser").equals("") &&
   ...
   !request.getAttribute("typeOfNewsMap").equals("")){
	user = (User)request.getAttribute("verifiedUser");
...
.button5:hover {
 ...
 <%
int contButCounter = listOfUserCategories.size();
%>
<script type="text/javascript">

var contButC = <%=contButCounter%>;
...
function myAlert(category, username, checki){
	//alert("hello");
	checkId = "butt" + checki;
	if($("#" + checkId).is(':checked')){
		//alert("checked" + category + " " + username + " " + checkId);
		
		$.ajax({
	        url: '<%= request.getContextPath()%>/dashboard/selectedCategory',//followCom
	        type: 'post',
	        dataType: 'html',
	        data: {
	        	category: category,
		...
		//alert("not checked" + category + " " + username + " " + checkId);
		
		$.ajax({
	        url: '<%= request.getContextPath()%>/dashboard/disSelecetedCategory',
	        type: 'post',
	        dataType: 'html',
	        data: {
	        	category: category,
...
<div id="main" class="columns clearfix" style = "text-align:center;" >
      <main id="content-column" class="content-column" role="main" style = "text-align:center;">
        <div class="content-inner" style = "text-align:center; text-align: center; width: 70%; margin-left: 15%; margin-right: 15%;">
        <br><br>
        <form id = "contForm" method = "post" action = "<%=request.getContextPath()%>/dashboard/" style = "text-align:center;">
              <button type = "submit" class="button button5">Continue</button>
        </form>
        <br><br>
        <h1 style = "text-align:center;">Welcome <%=user.getFullName() %></h1>
        <br>
        <h2>Select atleat 5 categories to continue</h2>
        <br>
        <br>
...
if(typeOfNewsList != null){
	int checki = 0;
	for(String newsListItem : typeOfNewsList){
		checki++;
%>
<div style = "width: 33%; float:left;">
<label class="container" style = "text-align:left;"><%=typeOfNewsMap.get(newsListItem) %>
  <input type="checkbox" value = "<%=newsListItem%>" id = "<%="butt" + checki %>" onclick = 'myAlert("<%=newsListItem %>", "<%=user.getUsername() %>", "<%=checki %>")'>
  <span class="checkmark" id = "<%="check" + checki%>"></span>
</label>
<br><br>
</div>
<%
