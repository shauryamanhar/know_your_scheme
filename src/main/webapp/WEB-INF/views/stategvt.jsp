...
ArrayList<String> statesForUrl = null;
...
if(session.getAttribute("knowYourSchemeUser") == null){
	if(request.getAttribute("listOfStateGovernmentSchemesByState") != null &&
			   ...
			   !request.getAttribute("mainState").equals("")){
				...
				statesForUrl = (ArrayList<String>)request.getAttribute("statesForUrl");
  ...
  <div class="block-content content"><div class="view view-most-search view-id-most_search view-display-id-block view-dom-id-e38b830996d06758e7d3042e35592d38">
	  ...
	  <!-- !Breadcrumbs -->
		<div class="bread_crumbs" > 
			<div class='wrapper'><a href="<%=request.getContextPath() %>/">Home</a><a href="<%=request.getContextPath()%>/schemes/">Schemes</a><%=mapOfstatesWithUrl.get(mainState) %></div>		</div> 
        <div class="content-inner wrapper">	
			<!-- !Messages and Help -->
									<div class="full_width_secction">
			  
								  <h1 class="static_page_head">State Government</h1>
				  ...
				  <section id="main-content">
					...
					<!-- !Main Content -->
						 ...
						 <div id="block-system-main" class="block block-system no-title" >          <div class="three-25-50-25 at-panel panel-display clearfix" >     <div class="region region-three-25-50-25-first">     <div class="region-inner clearfix">       <div class="panel-pane pane-block pane-npi-utility-left-menu-siblings left_sidebar test2 no-title block">    <div class="block-inner clearfix">                                <div class="block-content">        <div class="topic" id="leftside-bar">  					<div class="sidebar_head"><i class="fa fa-sitemap"></i><span> State Government Schemes </span></div>
						 <ul>
						 <%
						 if(mapOfstatesWithUrl != null && statesForUrl != null && mainState != null){
							 for(int ii = 0; ii < statesForUrl.size(); ii++){
								 if(statesForUrl.get(ii).equalsIgnoreCase("central-government-schemes")){
								  %>
									 <li style = "background-color:#eeeeee;"><a role="link" href="<%=request.getContextPath()%>/schemes/<%=statesForUrl.get(ii)%>"><%=mapOfstatesWithUrl.get(statesForUrl.get(ii))%></a></li>
								 <%	 
								 }else if(statesForUrl.get(ii).equalsIgnoreCase(mainState)){
									 %>
									 <li style = "background-color:#eeeeee;"><a role="link" href="<%=request.getContextPath()%>/schemes/State-Government/<%=statesForUrl.get(ii)%>"><%=mapOfstatesWithUrl.get(statesForUrl.get(ii))%></a></li>
									 <%
								 }else{
									 %>
									 <li><a role="link" href="<%=request.getContextPath()%>/schemes/State-Government/<%=statesForUrl.get(ii)%>"><%=mapOfstatesWithUrl.get(statesForUrl.get(ii))%></a></li>
									 <% 
								 } 
							 }
						 }
					               ...
					               <div class="block-content">        
					                   <div class="view view-level2-description view-id-level2_description view-display-id-block view-dom-id-13a1319c0ae258b76bc810598c2e6ecb">                      
					                       <div class="view-content">         
					                           ...
					                           <h2 class="pagetitle"> <span class="title-main"><%=mapOfstatesWithUrl.get(mainState) %></span> </h2>
					                           <ul style = "list-style-type: none;">
					                           <%
					                           if(listOfStateGovernmentSchemesByState != null){
					                        	   int i = 1;
					                        	   for(StatementGovernmentSchemes statementGovernmentSchemes : listOfStateGovernmentSchemesByState){
					                        		   if(statesForUrl.contains("central-government-schemes")){
					                        		   %>
					                        		   <li><a href = "<%=request.getContextPath()%>/schemes/central-government-schemes/<%=statementGovernmentSchemes.getSchemeName()%>"><h1><%=i++ %>. <%=statementGovernmentSchemes.getSchemeName().replaceAll("[-+.^:,]", " ") %></h1></a>
					                        		   <br>
					                        		   <ul style = "list-style-type: none;">
					                        		     <li><a href = "<%=request.getContextPath()%>/schemes/central-government-schemes/<%=statementGovernmentSchemes.getSchemeName()%>"><h3>Full Information</h3></a></li>
					                        		     <br>
					                        		   ...
					                        		   %>
					                        		   <li><a href = "<%=request.getContextPath()%>/schemes/State-Government/<%=mainState %>/<%=statementGovernmentSchemes.getSchemeName()%>"><h1><%=i++ %>. <%=statementGovernmentSchemes.getSchemeName().replaceAll("[-+.^:,]", " ") %></h1></a>
					                        		   <br>
					                        		   <ul style = "list-style-type: none;">
					                        		     <li><a href = "<%=request.getContextPath()%>/schemes/State-Government/<%=mainState %>/<%=statementGovernmentSchemes.getSchemeName()%>"><h3>Full Information</h3></a></li>
					                        		     <br>
					                        		     <li><a href = ""><h3>Related News</h3></a></li>
					                        		   </ul>
					                        		   </li>
